// ==UserScript==
// @name         GeoFS Better Crashes [ES]
// @namespace    https://github.com/
// @version      2.0.0
// @description  Explosión visual + sonido fuerte + shake exagerado al chocar en GeoFS. Incluye modo "Realistic" (corte seco a negro) y un panel de configuración (Alt+N) con sliders y reset.
// @author       You
...
(function () {
    ...
    // ============================================================
    // VALORES POR DEFECTO Y AJUSTES ACTUALES
    // ============================================================
    const DEFAULTS = Object.freeze({
        ...
        defaultFlashMs: 60,
        explosionVolume: 9,
        shakeIntensity: 45, // px de offset máximo; la rotación escala junto con esto
    ...
    // ratio fijo entre offset (px) y rotación (grados) del shake original: 3.5/45
    ...
    // ============================================================
    // AUDIO DEL SCRIPT / SILENCIO EN REALISTIC
    // ============================================================
    ...
    function triggerBetterCrash() {
        ...
        console.log(`💥 [Better Crashes] ¡Boom! (modo: ${SETTINGS.mode})`);
    ...
    // ============================================================
    // PANEL DE CONFIGURACIÓN (v2.0.0)
    // ============================================================
    let panel = null;
    let panelPos = null; // {left, top} tras el primer drag
    ...
    function showPanel() {
        ...
        panel.innerHTML = `
            ...
            <div id="bc-panel-body">
                <div class="bc-status" id="bc-status">Modo actual: ${modeLabel()}</div>

                <div class="bc-section-title">General</div>
                <div class="bc-row">
                    <button class="bc-mode-btn" id="bc-mode-btn">Cambiar a ${SETTINGS.mode === "realistic" ? "Default" : "Realistic"}</button>
                </div>

                <div class="bc-section-title">Visual</div>
                <label>Duración del flash - Default (ms) <span class="bc-val" id="bc-dflash-val">${SETTINGS.defaultFlashMs}</span></label>
                <input type="range" id="bc-dflash" min="20" max="300" step="10" value="${SETTINGS.defaultFlashMs}">

                <label>Duración del flash - Realistic (ms) <span class="bc-val" id="bc-rflash-val">${SETTINGS.realisticFlashMs}</span></label>
                <input type="range" id="bc-rflash" min="5" max="100" step="1" value="${SETTINGS.realisticFlashMs}">

                <label>Intensidad del camera shake (px) <span class="bc-val" id="bc-shake-val">${SETTINGS.shakeIntensity}</span></label>
                <input type="range" id="bc-shake" min="0" max="90" step="5" value="${SETTINGS.shakeIntensity}">

                <label>Cantidad de escombros <span class="bc-val" id="bc-debris-val">${SETTINGS.debrisCount}</span></label>
                <input type="range" id="bc-debris" min="10" max="200" step="10" value="${SETTINGS.debrisCount}">

                <label>Duración del tinte de fuego - Default (s) <span class="bc-val" id="bc-tint-val">${(SETTINGS.fireTintDurationMs / 1000).toFixed(0)}</span></label>
                <input type="range" id="bc-tint" min="1" max="15" step="1" value="${SETTINGS.fireTintDurationMs / 1000}">

                <div class="bc-section-title">Modo Realistic</div>
                <label>Duración del corte a negro (ms) <span class="bc-val" id="bc-cut-val">${SETTINGS.realisticCutMs}</span></label>
                <input type="range" id="bc-cut" min="100" max="500" step="5" value="${SETTINGS.realisticCutMs}">

                <div class="bc-section-title">Audio</div>
                <label>Volumen de la explosión <span class="bc-val" id="bc-vol-val">${SETTINGS.explosionVolume}</span></label>
                <input type="range" id="bc-vol" min="0" max="20" step="0.5" value="${SETTINGS.explosionVolume}">

                <button class="bc-reset-btn" id="bc-reset-btn">↺ Restaurar valores por defecto</button>
            </div>
        `;
        ...
        modeBtn.onclick = function () {
            ...
            statusEl.textContent = `Modo actual: ${modeLabel()}`;
            modeBtn.textContent = `Cambiar a ${SETTINGS.mode === "realistic" ? "Default" : "Realistic"}`;
            console.log(`💥 [Better Crashes] Modo cambiado a: ${SETTINGS.mode}`);
        };
        ...
        panel.querySelector("#bc-reset-btn").onclick = function () {
            ...
            explosionGain.gain.value = SETTINGS.explosionVolume;
            console.log("↺ [Better Crashes] Valores restaurados a los de por defecto.");
            panel.remove();
            panel = null;
            showPanel();
        };
    ...
    setInterval(() => {
        ...
        if (crashed && !wasCrashed) {
            ...
            console.log(`💥 [Better Crashes] Velocidad: ${lastSpeedKnots.toFixed(1)} kt | Factor de volumen: ${factor.toFixed(2)}x`);
            triggerBetterCrash();
